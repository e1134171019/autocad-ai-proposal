...
describe('UX hierarchy client comprehension contract', () => {
  ...
  it('ACT 05 手機技術圖不先佔滿畫面且仍保留責任與證據順序', () => {
    const intelligence = read(ACT05_PATH);
    const ruleEngine = intelligence.indexOf('C# / Rule Engine');
    const assistant = intelligence.indexOf('AI Assistant');
    const traceabilitySection = intelligence.indexOf('<section class="traceability"');
    ...
    expect(ruleEngine).toBeGreaterThanOrEqual(0);
    expect(assistant).toBeGreaterThanOrEqual(0);
    expect(traceabilitySection).toBeGreaterThanOrEqual(0);
    expect(ruleEngine).toBeLessThan(assistant);
    expect(assistant).toBeLessThan(traceabilitySection);
  });
  ...
  it('CAD ByLayer 與操作狀態色仍保留', () => {
    ...
    ['--cad-layer-exterior:', '--cad-layer-shaft:', '--cad-layer-dimension:', '--cad-selection:', '--cad-grip:']
